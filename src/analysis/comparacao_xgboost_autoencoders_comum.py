...
SAVE_PATH = r"C:\...\src\plots_testes_comum"
CANAL_ALVO = "4"
...
# 1. CARREGAMENTO 
processed_data = []
print("Iniciando carregamento do Sensor Comum (Audível)...")
...
for pasta in pastas:
    ...
    if drill_match:
        ...
        for root, _, files in os.walk(caminho_pasta):
            # Filtra apenas se estiver na pasta reg_mics
                ...
                for f in files:
                    if f.lower().endswith(".wav") and (f"ch{CANAL_ALVO}" in f.lower() or f"tr{CANAL_ALVO}" in f.lower()):
                        ...
                        if h_match:
                            ...
                            dict_audible[hole_idx] = os.path.join(root, f)

        holes = sorted(list(dict_audible.keys()))
        ...
        if len(holes) > N_NORMAL + 2:
            ...
            print(f"Lendo Drill {drill_num:02d} - {n_total} furos encontrados (Audível).")
            for i, h_idx in enumerate(holes):
                feat = extract_features_46d(dict_audible[h_idx])
...
for TEST_DRILL in lista_brocas:
    ...
    plt.title(f"Drill {TEST_DRILL:02d} - Common Mic (Audible)")
    plt.grid(True, linestyle=':', alpha=0.4)
    plt.tight_layout()

    plt.savefig(os.path.join(SAVE_PATH, f"drill_{TEST_DRILL:02d}_common_mic.png"), dpi=600, bbox_inches='tight')
    plt.close()

print(f"\n[SUCESSO] Monitoramento concluído para o Sensor Comum! Gráficos em: {SAVE_PATH}")