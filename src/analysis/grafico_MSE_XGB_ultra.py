...
for drill in sorted(df["drill"].unique()):
    ...
    # Pegamos a probabilidade contínua da classe 1 (score) salva pelo pipeline ultrassônico
    coluna_prob = "score"
...
ax.set_title("Average XGBoost Failure Probability (Microfone Ultrassônico)")
...
plt.savefig("xgboost_mean_prob_std_ultrasonic.pdf", dpi=600, bbox_inches="tight")
plt.savefig("xgboost_mean_prob_std_ultrasonic.png", dpi=300, bbox_inches="tight")
plt.show()

print("Gráfico de evolução média do XGBoost (Ultrassônico) gerado com sucesso!")