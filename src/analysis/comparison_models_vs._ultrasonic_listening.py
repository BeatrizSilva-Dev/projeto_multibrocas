...
for drill_folder in os.listdir(ROOT_DATASET):
    ...
    if len(X_drill) > N_NORMAL:
        ...
        drills_data[clean_key] = {"X": np.array(X_drill), "y": np.array(y_drill), "holes": hole_indices}
...
for drill_test in drills_data:
    ...
    for i in range(mlp_window - 1, n_holes):
        if np.all(holes_above_mlp[i - (mlp_window - 1):i + 1] == 1): pred_mlp[i] = 1
    ...
    for i in range(n_holes):
        comparison_records.append({
            "drill": drill_test, "hole": test_holes[i],
