...
# ==============================================================================
# 🔒 CONFIGURAÇÕES DE DETERMINISMO E REPRODUTIBILIDADE EM CPU (BLINDADO)
# ==============================================================================
...
# --- CONFIGURAÇÕES BASE DO SEU FRAMEWORK ---
...
MIC_A = "reg_mics"  # ➔ FOCO EXCLUSIVO: Microfone Comum / Audível
CANAL_ALVO = "4"
N_NORMAL = 5
N_MFCC = 20
SEQ_LEN = 10
...
# --- 1. PROCESSAMENTO EXCLUSIVO DO MICROFONE COMUM ---
...
drills_data = {}
print("Carregando base do MICROFONE COMUM para alinhamento multimodelo...")
for drill_folder in os.listdir(ROOT_DATASET):
    ...
    for i, hole in enumerate(common_holes):
        try:
            ...
            # ➔ ALTERAÇÃO CRÍTICA: Vetor puramente audível (44 dimensões)
...
# --- 2. EXECUÇÃO DO LOOP LODO TOTALMENTE SINCRONIZADO ---
registros_comparacao = []
dados_mlp_contínuos = []
dados_lstm_contínuos = []
dados_xgb_contínuos = []

print("Executando inferência combinada estrita para Microfone Comum...")
...
for drill_test in drills_data:
    ...
    # --------------------------------------------------------------------------
    # [MODELO 1] MLP Autoencoder - Escala por Broca Estável
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # [MODELO 2] LSTM Autoencoder - Parâmetros Originais Ajustados (50 Épocas)
    # --------------------------------------------------------------------------
    ...
    # --------------------------------------------------------------------------
    # [MODELO 3] XGBoost - Parâmetros Históricos e Threshold Percentil
    # --------------------------------------------------------------------------
...
# --- 3. MERGE COM DIÁRIO DE ESCUTA ACÚSTICA ---
...
# --- 4. GERAR O GRÁFICO COMPARATIVO TRI-MODELO DE BARRAS ---
plt.rcParams.update({
...
# ➔ SALVAMENTO RECALIBRADO PARA MICROFONE COMUM
...
# ==============================================================================
# EXPORTAÇÃO DOS REPOSITÓRIOS EXCLUSIVOS DO SENSOR COMUM (SFIXO _comum)
# ==============================================================================
print("\n💾 Exportando repositórios contínuos do microfone comum para gráficos de rampa...")
...
print("\n[SUCESSO] Pipeline exclusivo do sensor audível concluído e salvo!")