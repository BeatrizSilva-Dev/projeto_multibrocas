...
SAVE_PATH = r"C:\Users\...\plots\plots_todos_testes_hibrido"
N_NORMAL = 5
...
# 2. LOOP DE PLOTAGEM FURO A FURO POR UNIDADE DE FERRAMENTA
for drill in lista_brocas:
    ...
    # Extração dos thresholds estritos calculados 
    ...
    # CORREÇÃO: Filtra os valores maiores que zero para ignorar a janela morta inicial do LSTM
    ...
    # Marcação visual da Fase Crítica nos últimos 20% do ciclo de vida
    ...
    ax1.plot(furos, sub_lstm['hybrid_mse'], color='#7209b7', marker='o', markersize=2.2,
             linewidth=1.0, label='LSTM-AE MSE')
    ax1.plot(furos, sub_mlp['hybrid_mse'], color='#008000', marker='v', markersize=2.2,
             linewidth=0.9, linestyle='--', label='MLP-AE MSE')

    # Linhas de Threshold dos Autoencoders
    ax1.axhline(y=thresh_lstm, color='#7209b7', linestyle=':', linewidth=0.8, alpha=0.7,
                label=f'Thresh LSTM ({thresh_lstm:.2e})')
    ax1.axhline(y=thresh_mlp, color='#008000', linestyle=':', linewidth=0.8, alpha=0.7,
    ...
    # Configuração de Probabilidade do XGBoost (Eixo Direito)
    ax2 = ax1.twinx()
    ax2.set_ylim(-0.05, 1.05)

    # Força o eixo X a mostrar apenas furos inteiros
    ...
    ax2.plot(furos, sub_xgb['score'], color='#e67e22', linestyle='-.', marker='s',
             markersize=2, linewidth=0.9, label='XGB Conf.')

    ax2.axhline(y=thresh_xgb, color='#e67e22', linestyle=':', linewidth=0.8, alpha=0.7,
    ...
    # Salvamento duplo limpo e mapeado por string com margens zeradas
    pdf_filename = f"{drill}_multimodel_monitoring.pdf"
    png_filename = f"{drill}_multimodel_monitoring.png"

    plt.savefig(os.path.join(SAVE_PATH, pdf_filename), dpi=600, bbox_inches='tight', pad_inches=0.005)
    plt.savefig(os.path.join(SAVE_PATH, png_filename), dpi=300, bbox_inches='tight', pad_inches=0.005)
    plt.close()

print(f"\n[SUCESSO] Novos gráficos ultra-compactos gerados! Salvos em: {SAVE_PATH}")