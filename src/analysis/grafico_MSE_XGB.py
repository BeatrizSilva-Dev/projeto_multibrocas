...
df = pd.read_csv("resultados_xgboost_hibrido.csv")
...
for drill in sorted(df["drill"].unique()):
    ...
    # Pegamos a probabilidade contínua da classe 1 (score) salva pelo Mestre
    coluna_prob = "score"
...
# Truncamos os limites entre 0 e 1 para o desvio padrão não estourar os limites físicos do gráfico
...
ax.set_ylim(-0.05, 1.05) # Margem de segurança para as linhas não colarem no teto
ax.set_xlabel("Tool Life (%)")
ax.set_ylabel("XGBoost Failure Probability (Score)")
ax.set_title("Average XGBoost Failure Probability Across All Drills")
...
ax.text(
    81,
    ymax * 0.85, # Alinha o texto de forma limpa no topo da escala linear de probabilidade
    "Critical Phase",
    fontsize=9,
    weight="bold"
)
...
plt.savefig("xgboost_mean_prob_std.pdf", dpi=600, bbox_inches="tight")
plt.savefig("xgboost_mean_prob_std.png", dpi=300, bbox_inches="tight")
plt.show()

print("Gráfico de evolução de probabilidade do XGBoost gerado de forma padronizada!")