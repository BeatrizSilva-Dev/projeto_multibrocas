...
df_listening = pd.read_csv(LISTENING_FILE)
...
df_listening['drill'] = df_listening['drill'].apply(clean_drill_name)
...
for drill_folder in os.listdir(ROOT_DATASET):
    ...
    if len(X_drill) > N_NORMAL:
        clean_key = clean_drill_name(drill_folder)
        drills_data[clean_key] = {
            "X": np.array(X_drill),
            "y": np.array(y_drill),
            "holes": hole_indices
        }
...
for drill_test in drills_data:
    ...
    test_holes = drills_data[drill_test]["holes"]
    n_holes = len(X_test)
    y_test = drills_data[drill_test]["y"]

    global_scaler = StandardScaler()
    X_train_sc = global_scaler.fit_transform(X_train)
    X_test_sc_xgb = global_scaler.transform(X_test)
    ...
    mlp_test_errors = np.mean(
        (X_test_sc_mlp - mlp_ae.predict(X_test_sc_mlp)) ** 2,
        axis=1
    )
    ...
    # INDIVIDUAL CHRONOLOGICAL PERSISTENCE WINDOW FILTERS
    ...
    for i in range(1, n_holes):
        if holes_above_xgb[i] == 1 and holes_above_xgb[i - 1] == 1:
            pred_xgb[i] = 1

    # MLP-AE (Window = 10)
    mlp_window = 10
    for i in range(mlp_window - 1, n_holes):
        if np.all(holes_above_mlp[i - (mlp_window - 1):i + 1] == 1):
            pred_mlp[i] = 1

    # LSTM-AE (Window = 8)
    lstm_window = 8
    for i in range(lstm_window - 1, n_holes):
        if np.all(holes_above_lstm[i - (lstm_window - 1):i + 1] == 1):
            pred_lstm[i] = 1

    for i in range(n_holes):
        hole_id = test_holes[i]

        continuous_mlp_data.append({
            "drill": drill_test,
            "hole": hole_id,
            "hybrid_mse": float(mlp_test_errors[i]),
            "prediction": int(pred_mlp[i]),
            "label": int(y_test[i])
        })
        ...
        continuous_lstm_data.append({
            "drill": drill_test,
            "hole": hole_id,
            "hybrid_mse": float(lstm_test_errors[i]),
            "prediction": int(pred_lstm[i]),
            "label": int(y_test[i])
        })
    ...
    for i in range(n_holes):
        comparison_records.append({
            "drill": drill_test,
            "hole": test_holes[i],
            "alarm_xgb": int(pred_xgb[i]),
            "alarm_mlp": int(pred_mlp[i]),
            "alarm_lstm": int(pred_lstm[i])
        })

df_models = pd.DataFrame(comparison_records)
df_final = pd.merge(df_models, df_listening, on=['drill', 'hole'])
...
# Grouping and percentage mean calculation
grouped = df_final.groupby('human_severity_score')[['alarm_xgb', 'alarm_mlp', 'alarm_lstm']].mean() * 100

grouped.columns = [
    'Supervised (XGBoost)',
    'Semi-Supervised (MLP-AE)',
    'Semi-Supervised (LSTM-AE)'
]
...
grouped.index = [severity_labels[i] for i in grouped.index]

fig, ax = plt.subplots(figsize=(3.5, 3.8))

original_colors = ['#e67e22', '#008000', '#7209b7']
grouped.plot(
    kind='bar',
    ax=ax,
...
ax.set_ylabel("Alarm Sensitivity, $S_{alarm}$ (%)", fontsize=9)
...
plt.savefig("multimodel_audio_severity_comparison.pdf", dpi=600)
plt.savefig("multimodel_audio_severity_comparison.png", dpi=300)

plt.show()
plt.close()

df_mlp_real = pd.DataFrame(continuous_mlp_data)
df_mlp_real.to_csv("results_autoencoder_hybrid.csv", index=False)

df_lstm_real = pd.DataFrame(continuous_lstm_data)
df_lstm_real.to_csv("results_LSTM_hybrid.csv", index=False)

df_xgb_real = pd.DataFrame(continuous_xgb_data)
df_xgb_real.to_csv("results_xgboost_hybrid.csv", index=False)
