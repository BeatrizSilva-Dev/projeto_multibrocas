...
for drill_folder in os.listdir(ROOT_DATASET):
    ...
    for root, _, fs in os.walk(path):
        if MIC_B not in root.lower(): continue
    ...
    holes_sorted = sorted(dict_b.keys())
    ...
    for i, hole in enumerate(holes_sorted):
        try:
            y_load, sr_load = librosa.load(dict_b[hole], sr=None)
...
if regional_results:
    ...
    plt.savefig("matriz_xgboost_ultrassonico.pdf", dpi=600, bbox_inches='tight')
    plt.show()

    # Curva ROC
    plt.figure(figsize=(3.5, 3))
    plt.plot(fpr, tpr, label=f'AUC = {auc_real:.2f}', color='darkorange')
    plt.plot([0,1], [0,1], linestyle='--', color='gray')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.legend()
    plt.tight_layout()
    plt.savefig("roc_xgboost_ultrassonico.pdf", dpi=600, bbox_inches='tight')
    plt.show()
