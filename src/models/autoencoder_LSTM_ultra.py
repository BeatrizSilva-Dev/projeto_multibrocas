...
from tensorflow.keras import backend as K  # Adicionado para gerenciamento ativo de memória
...
for drill_folder in os.listdir(ROOT_DATASET):
    ...
    # Furos disponíveis no sensor ultrassônico
    holes_available = sorted(dict_b.keys())

    if len(holes_available) <= N_NORMAL + 2:
        continue

    # Remove último furo
    holes_available = holes_available[:-1]

    X = []
    for hole in holes_available:
        try:
            y_b, sr_b = librosa.load(dict_b[hole], sr=None)
            feat_b = extract_features(y_b, sr_b)
            X.append(feat_b) # Apenas 46 características (MFCCs + RMS)
        except:
            continue
...
# 2. VALIDAÇÃO LODO
regional_results = []
lead_times = []  # Lista para armazenar o lead-time de cada broca
...
for drill_name, X in drills_data.items():
    ...
    # CORREÇÃO: Remoção dos zeros do warm-up temporal para evitar contaminação no cálculo do limiar
    ...
    if len(alert_indices) > 0:
        first_alert_hole = alert_indices[0] + 1  # Base 1 (Furo físico real)
    ...
    for i in range(n_holes):
        export_data.append({
            ...
            'prediction': preds[i],
            'label': labels[i],
            'drill_lead_time': drill_lead_time  # Exportação da métrica para análise comparativa
        })
...
if regional_results:
    ...
    auc_global = roc_auc_score(all_labels, all_scores)
    ...
    plt.tight_layout()
    plt.savefig("matriz_lstm_ultrassonico.pdf", dpi=600, bbox_inches='tight')
    plt.show()

    # Curva ROC
    fpr, tpr, _ = roc_curve(all_labels, all_scores)
    plt.figure(figsize=(3.5, 3))
    plt.plot(fpr, tpr, color='orange', linewidth=1.5, label=f'AUC = {auc_global:.2f}')
    ...
    plt.tight_layout()
    plt.savefig("roc_lstm_ultrassonico.pdf", dpi=600, bbox_inches='tight')
    plt.show()

    df_export = pd.DataFrame(export_data)
    df_export.to_csv("resultados_lstm_ultrassonico.csv", index=False)
    print("\n[OK] CSV 'resultados_lstm_ultrassonico.csv' exportado.")
