...
arquivo_comum = "resultados_xgboost_comum.csv"
if not os.path.exists(arquivo_comum):
    raise FileNotFoundError(f"Gere o arquivo '{arquivo_comum}' rodando o pipeline comum primeiro!")

df_xgb = pd.read_csv(arquivo_comum)
...
print(f"\nMÉTRICAS REAIS DO XGBOOST (MICROFONE COMUM)")
...
plt.savefig("matriz_xgboost_comum_final.pdf", dpi=600, bbox_inches='tight', pad_inches=0.01)
plt.savefig("matriz_xgboost_comum_final.png", dpi=300, bbox_inches='tight', pad_inches=0.01)
plt.show()

# 5. PLOTAGEM DA CURVA ROC DO XGBOOST
...
ax.set_title('ROC Curve - XGBoost Comum', fontweight='bold', fontsize=10, pad=10)
ax.legend(loc='lower right', fontsize=9)
ax.grid(True, linestyle=':', alpha=0.3)
plt.tight_layout()

plt.savefig("roc_xgboost_comum.pdf", dpi=600, bbox_inches='tight')
plt.savefig("roc_xgboost_comum.png", dpi=300, bbox_inches='tight')
plt.show()

print("Gráficos e métricas individuais do XGBoost Comum exportados com consistência absoluta!")