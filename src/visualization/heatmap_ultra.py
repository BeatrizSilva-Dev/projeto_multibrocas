...
print("Processando todas as brocas (Apenas Microfone Ultrassônico)...")
...
for pasta in os.listdir(ROOT_DATASET):
    ...
    for root, _, fs in os.walk(pasta_path):
        for f in fs:
            ...
            # Filtra estritamente o diretório do sensor ultrassônico
            if MIC_B in root.lower():
                dict_b[hole] = full_path

    holes = sorted(list(dict_b.keys()))

    if len(holes) < 10:
        print(f"DESCARTADA -> {pasta} | Furos ultrassônico = {len(holes)}")
        continue
    ...
    for hole in holes:
        try:
            ...
            energia_ultrasonic = np.clip(np.mean(mfcc_b, axis=1), -80, 50)
            mfcc_drill.append(energia_ultrasonic)
        except:
            continue
...
ax.set_title("Average MFCC Energy Evolution Across All Drills (Microfone Ultrassônico)", fontsize=11, pad=12, weight='bold')
ax.set_xlabel("Tool Life (%)", fontsize=10)
ax.set_ylabel("Representação de Faixas de Frequência (MFCCs)", fontsize=10)

plt.tight_layout()
plt.savefig("heatmap_espectral_drill_ultrasonic.pdf", dpi=600, bbox_inches='tight')
plt.savefig("heatmap_espectral_drill_ultrasonic.png", dpi=300, bbox_inches='tight')
plt.show()

print("O mapa de calor espectral do microfone ultrassônico foi gerado!")