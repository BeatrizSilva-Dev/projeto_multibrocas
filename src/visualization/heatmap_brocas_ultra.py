...
df_xgb  = pd.read_csv("resultados_xgboost_ultrassonico.csv")
df_mlp  = pd.read_csv("resultados_autoencoder_ultrassonico.csv")
df_lstm = pd.read_csv("resultados_lstm_ultrassonico.csv")
...
def plotar_heatmap_categorico(matriz, nomes, titulo, arquivo_saida):
    ...
    plt.tight_layout()
    plt.savefig(arquivo_saida + "_ultrasonic.pdf", dpi=600, bbox_inches="tight")
    plt.savefig(arquivo_saida + "_ultrasonic.png", dpi=300, bbox_inches="tight")
    plt.close()


print("Plotando painéis de risco ultrassônicos...")
plotar_heatmap_categorico(matriz_xgb, nomes, "XGBoost - Monitoramento Temporal (Microfone Ultrassônico)", "heatmap_risk_xgboost")
plotar_heatmap_categorico(matriz_mlp, nomes, "MLP-AE - Monitoramento Temporal (Microfone Ultrassônico)", "heatmap_risk_mlp")
plotar_heatmap_categorico(matriz_lstm, nomes, "LSTM-AE - Monitoramento Temporal (Microfone Ultrassônico)", "heatmap_risk_lstm")

print("\n[SUCESSO] Os 3 heatmaps do microfone ultrassônico foram gerados e salvos!")