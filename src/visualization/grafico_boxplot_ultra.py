...
def gerar_boxplot_ultrassonico_ieee():
    try:
        ...
        # 1. Definição dos caminhos dos CSVs específicos do sensor ULTRASSÔNICO
        ...
        def extrair_f1_real_por_broca(df, modelo):
            resultados_f1 = []
            # Detecta o nome correto da coluna de label adotada em cada script
            ...
            for drill in sorted(df['drill'].unique()):
                ...
                # zero_division=0 evita warnings em brocas com comportamento anômalo extremo
                score = f1_score(y_true, y_pred, zero_division=0)
                resultados_f1.append({'drill': drill, 'f1_score': score, 'modelo': modelo})
            return pd.DataFrame(resultados_f1)

        # 3. Processamento direto (Mapeamento livre de bugs matemáticos)
        ...
        # Paleta de tons quentes específicos do ultrassom (Oranges)
